...
import pygame, sys, events, os, tile_dictionaries, filepaths, game_state, menus, battle, npcs, functions
...
def interaction(sprite):
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN: #talking to NPCs
			if event.key == pygame.K_z:
				if sprite.orient == 'up':
					...
					if x.tile == future_tile_number:
						if sprite.orient == 'up':
							...
							if x.orient != "left":
								x.orient = "left"
						print x.text
						game_state.current_npc_text = x.text
			...
			if initial_orientation == 'up':
				if sprite.hold_time > 4 or sprite.orient == 'up':
					...
					if future_tile_number in range(1, Tile.total_tiles + 1):
						...
						l = []
						for x in Npc.List:
							l.append(x.get_number())	
						if future_tile.walkable == True and future_tile_number not in l:
